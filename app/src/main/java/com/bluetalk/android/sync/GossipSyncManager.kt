...
class GossipSyncManager(
    ...
    fun start() {
        ...
        cleanupJob = scope.launch(Dispatchers.IO) {
            while (isActive) {
                try {
                    delay(com.bluetalk.android.util.AppConstants.Sync.CLEANUP_INTERVAL_MS)
    ...
    fun onPublicPacketSeen(packet: BlueTalkPacket) {
        ...
        if (isBroadcastMessage) {
            ...
            if (age > com.bluetalk.android.util.AppConstants.Mesh.STALE_PEER_TIMEOUT_MS) {
                Log.d(TAG, "Ignoring stale ANNOUNCE (age=${age}ms > ${com.bluetalk.android.util.AppConstants.Mesh.STALE_PEER_TIMEOUT_MS}ms)")
                return
            }
    ...
    private fun sendRequestSync() {
        ...
        val packet = BlueTalkPacket(
            ...
            payload = payload,
            ttl = com.bluetalk.android.util.AppConstants.SYNC_TTL_HOPS // neighbors only
        )
        // Sign and broadcast
    ...
    private fun sendRequestSyncToPeer(peerID: String) {
        ...
        val packet = BlueTalkPacket(
            ...
            timestamp = System.currentTimeMillis().toULong(),
            payload = payload,
            ttl = com.bluetalk.android.util.AppConstants.SYNC_TTL_HOPS // neighbor only
        )
    ...
    fun handleRequestSync(fromPeerID: String, request: RequestSyncPacket) {
        ...
        for ((_, pair) in latestAnnouncementByPeer.entries) {
            ...
            if (!mightContain(idBytes)) {
                ...
                val toSend = pkt.copy(ttl = com.bluetalk.android.util.AppConstants.SYNC_TTL_HOPS)
