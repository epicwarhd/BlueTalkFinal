package com.bluetalk.android.protocol
...
data class BlueTalkPacket(
    ...
    fun toBinaryDataForSigning(): ByteArray? {
        ...
        val unsignedPacket = BlueTalkPacket(
            version = version,
            type = type,
            senderID = senderID,
            ...
            route = route,
            ttl = com.bluetalk.android.util.AppConstants.SYNC_TTL_HOPS // Use fixed TTL=0 for signing to ensure relay compatibility
        )
        return BinaryProtocol.encode(unsignedPacket)
    }

    companion object {
        fun fromBinaryData(data: ByteArray): BlueTalkPacket? {
...
object BinaryProtocol {
    ...
    fun encode(packet: BlueTalkPacket): ByteArray? {
        try {
    ...
    fun decode(data: ByteArray): BlueTalkPacket? {
     ...
     */
    private fun decodeCore(raw: ByteArray): BlueTalkPacket? {
        try {
            ...
            if (payloadLength > com.bluetalk.android.util.AppConstants.Protocol.MAX_PAYLOAD_LENGTH.toUInt()) {
                Log.w("BinaryProtocol", "Payload length ${payloadLength} exceeds maximum allowed (${com.bluetalk.android.util.AppConstants.Protocol.MAX_PAYLOAD_LENGTH})")
                return null
            }
