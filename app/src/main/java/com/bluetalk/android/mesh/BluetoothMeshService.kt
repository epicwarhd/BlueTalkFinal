...
import com.bluetalk.android.sync.GossipSyncManager
...
class BluetoothMeshService(private val context: Context) {
    ...
    private val serviceNotificationManager = com.bluetalk.android.ui.NotificationManager(
        ...
        com.bluetalk.android.util.NotificationIntervalManager()
    )
        ...
        gossipSyncManager = GossipSyncManager(
            ...
            configProvider = object : GossipSyncManager.ConfigProvider {
                override fun seenCapacity(): Int = try {
                    com.bluetalk.android.ui.debug.DebugPreferenceManager.getSeenPacketCapacity(500)
                } catch (_: Exception) { 500 }

                override fun gcsMaxBytes(): Int = try {
                    com.bluetalk.android.ui.debug.DebugPreferenceManager.getGcsMaxFilterBytes(400)
                } catch (_: Exception) { 400 }

                override fun gcsTargetFpr(): Double = try {
                    com.bluetalk.android.ui.debug.DebugPreferenceManager.getGcsFprPercent(1.0) / 100.0
    ...
    private fun setupDelegates() {
        ...
        peerManager.delegate = object : PeerManagerDelegate {
            override fun onPeerListUpdated(peerIDs: List<String>) {
                ...
                try { com.bluetalk.android.services.AppStateStore.setPeers(peerIDs) } catch (_: Exception) { }
            ...
            override fun onPeerRemoved(peerID: String) {
                ...
                try { com.bluetalk.android.services.meshgraph.MeshGraphService.getInstance().removePeer(peerID) } catch (_: Exception) { }
        ...
        messageHandler.delegate = object : MessageHandlerDelegate {
            ...
            override fun updatePeerIDBinding(newPeerID: String, nickname: String,
                ...
                try {
                    com.bluetalk.android.favorites.FavoritesPersistenceService.shared.findNostrPubkey(publicKey)?.let { npub ->
                        com.bluetalk.android.favorites.FavoritesPersistenceService.shared.updateNostrPublicKeyForPeerID(newPeerID, npub)
                    }
                } catch (_: Exception) { }
                ...
                try {
                    when {
                        message.isPrivate -> {
                            ...
                            if (peer.isNotEmpty()) com.bluetalk.android.services.AppStateStore.addPrivateMessage(peer, message)
                        }
                        message.channel != null -> {
                            com.bluetalk.android.services.AppStateStore.addChannelMessage(message.channel!!, message)
                        }
                        else -> {
                            com.bluetalk.android.services.AppStateStore.addPublicMessage(message)
                        }
                    }
                } catch (_: Exception) { }
                ...
                if (delegate == null && message.isPrivate) {
                    try {
                        ...
                        if (senderPeerID != null) {
                            ...
                            val preview = com.bluetalk.android.ui.NotificationTextUtils.buildPrivateMessagePreview(message)
            ...
            override fun handleAnnounce(routed: RoutedPacket) {
                serviceScope.launch {
                    ...
                    if (deviceAddress != null && pid != null) {
                        ...
                        val isDirect = routed.packet.ttl == com.bluetalk.android.util.AppConstants.MESSAGE_TTL_HOPS
        ...
        override fun onPacketReceived(packet: BlueTalkPacket, peerID: String, device: android.bluetooth.BluetoothDevice?) {
            ...
            override fun onDeviceConnected(device: android.bluetooth.BluetoothDevice) {
                ...
                try {
                    ...
                    com.bluetalk.android.ui.debug.DebugSettingsManager.getInstance()
            ...
            serviceScope.launch {
                ...
                if (encryptionService.hasEstablishedSession(recipientPeerID)) {
                    try {
                        ...
                        val noisePayload = com.bluetalk.android.model.NoisePayload(
                            type = com.bluetalk.android.model.NoisePayloadType.FILE_TRANSFER,
                            data = filePayload
                        )
                        ...
                        val packet = BlueTalkPacket(
                            ...
                            payload = encrypted,
                            signature = null,
                            ttl = com.bluetalk.android.util.AppConstants.MESSAGE_TTL_HOPS
                        )
        ...
        serviceScope.launch {
            ...
            if (encryptionService.hasEstablishedSession(recipientPeerID)) {
                try {
                    ...
                    val privateMessage = com.bluetalk.android.model.PrivateMessagePacket(
                    ...
                    val messagePayload = com.bluetalk.android.model.NoisePayload(
                        type = com.bluetalk.android.model.NoisePayloadType.PRIVATE_MESSAGE,
                        data = tlvData
                    )
    ...
    fun sendReadReceipt(messageID: String, recipientPeerID: String, readerNickname: String) {
        serviceScope.launch {
            ...
            val geo = runCatching { com.bluetalk.android.services.MessageRouter.tryGetInstance() }.getOrNull()
            val isGeoAlias = try {
                val map = com.bluetalk.android.nostr.GeohashAliasRegistry.snapshot()
                map.containsKey(recipientPeerID)
            } catch (_: Exception) { false }
            if (isGeoAlias && geo != null) {
                geo.sendReadReceipt(com.bluetalk.android.model.ReadReceipt(messageID), recipientPeerID)
                return@launch
            }

            try {
                // Avoid duplicate read receipts: check persistent store first
                val seenStore = try { com.bluetalk.android.services.SeenMessageStore.getInstance(context.applicationContext) } catch (_: Exception) { null }
                ...
                val readReceiptPayload = com.bluetalk.android.model.NoisePayload(
                    type = com.bluetalk.android.model.NoisePayloadType.READ_RECEIPT,
                ...
                val packet = BlueTalkPacket(
                    ...
                    payload = encrypted,
                    signature = null,
                    ttl = com.bluetalk.android.util.AppConstants.MESSAGE_TTL_HOPS // Same TTL as iOS messageTTL
                )
    ...
    fun sendBroadcastAnnounce() {
        ...
        serviceScope.launch {
            val nickname = try { com.bluetalk.android.services.NicknameProvider.getNickname(context, myPeerID) } catch (_: Exception) { myPeerID }
            ...
            try {
                ...
                if (directPeers.isNotEmpty()) {
                    val gossip = com.bluetalk.android.services.meshgraph.GossipTLV.encodeNeighbors(directPeers)
                    tlvPayload = tlvPayload + gossip
                }
                // Always update our own node in the mesh graph with the neighbor list we used
                try {
                    com.bluetalk.android.services.meshgraph.MeshGraphService.getInstance()
        ...
        return try {
            ...
            val withRoute = try {
                ...
                if (rec != null && !rec.contentEquals(SpecialRecipients.BROADCAST)) {
                    ...
                    val path = com.bluetalk.android.services.meshgraph.RoutePlanner.shortestPath(myPeerID, dest)
