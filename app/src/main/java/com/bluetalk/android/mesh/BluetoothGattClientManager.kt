...
import com.bluetalk.android.ui.debug.DebugSettingsManager
import com.bluetalk.android.ui.debug.DebugScanResult
...
class BluetoothGattClientManager(
    ...
    fun start(): Boolean {
        ...
        try {
            if (!com.bluetalk.android.ui.debug.DebugSettingsManager.getInstance().gattClientEnabled.value) {
    ...
    fun onScanStateChanged(shouldScan: Boolean) {
        val enabled = try { com.bluetalk.android.ui.debug.DebugSettingsManager.getInstance().gattClientEnabled.value } catch (_: Exception) { true }
    ...
    private fun handleScanResult(result: ScanResult) {
        ...
        val dbg = try { com.bluetalk.android.ui.debug.DebugSettingsManager.getInstance() } catch (_: Exception) { null }
    ...
    private fun connectToDevice(device: BluetoothDevice, rssi: Int, peerID: String? = null) {
        ...
        val deviceAddress = device.address
        Log.i(TAG, "Connecting to bluetalk device: $deviceAddress (peerID: $peerID)")
