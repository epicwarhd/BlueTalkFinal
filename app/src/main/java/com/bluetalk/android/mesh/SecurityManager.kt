...
class SecurityManager(private val encryptionService: EncryptionService, private val myPeerID: String) {
    ...
    companion object {
        ...
        private const val MESSAGE_TIMEOUT = com.bluetalk.android.util.AppConstants.Security.MESSAGE_TIMEOUT_MS // 5 minutes (same as iOS)
        private const val CLEANUP_INTERVAL = com.bluetalk.android.util.AppConstants.Security.CLEANUP_INTERVAL_MS // 5 minutes
        private const val MAX_PROCESSED_MESSAGES = com.bluetalk.android.util.AppConstants.Security.MAX_PROCESSED_MESSAGES
        private const val MAX_PROCESSED_KEY_EXCHANGES = com.bluetalk.android.util.AppConstants.Security.MAX_PROCESSED_KEY_EXCHANGES
    }
    ...
    fun validatePacket(packet: BlueTalkPacket, peerID: String): Boolean {
        ...
        if (processedMessages.contains(messageID)) {
            ...
            val isFreshAnnounce = messageType == MessageType.ANNOUNCE &&
                    packet.ttl >= com.bluetalk.android.util.AppConstants.MESSAGE_TTL_HOPS
    ...
    private fun verifyPacketSignature(packet: BlueTalkPacket, peerID: String): Boolean {
        try {
            ...
            if (MessageType.fromValue(packet.type) == MessageType.ANNOUNCE) {
                ...
                try {
                    val announcement = com.bluetalk.android.model.IdentityAnnouncement.decode(packet.payload)
