...
class MeshDelegateHandler(
                ...
                if (state.getJoinedChannelsValue().contains(message.channel)) {
                    ...
                    val viewingGeohash = try {
                        if (channel.startsWith("geo:")) {
                            ...
                            selected is com.bluetalk.android.geohash.ChannelID.Location && selected.channel.geohash.equals(geo, ignoreCase = true)
                        } else false
    ...
    override fun didUpdatePeerList(peers: List<String>) {
        coroutineScope.launch {
            ...
            runCatching { com.bluetalk.android.services.MessageRouter.tryGetInstance()?.onPeersUpdated(peers) }
                ...
                if (isNostrAlias || isNoiseHex) {
                    ...
                    val canonical = com.bluetalk.android.services.ConversationAliasResolver.resolveCanonicalPeerID(
                        ...
                        nostrPubHexForAlias = { alias ->
                            ...
                            if (com.bluetalk.android.nostr.GeohashAliasRegistry.contains(alias)) {
                                com.bluetalk.android.nostr.GeohashAliasRegistry.get(alias)
                            } else {
                                // Best-effort: derive pub hex from favorites mapping for mesh nostr_ aliases
                                val prefix = alias.removePrefix("nostr_")
                                val favs = try { com.bluetalk.android.favorites.FavoritesPersistenceService.shared.getOurFavorites() } catch (_: Exception) { emptyList() }
                                favs.firstNotNullOfOrNull { rel ->
                                    rel.peerNostrPublicKey?.let { s ->
                                        runCatching { com.bluetalk.android.nostr.Bech32.decode(s) }.getOrNull()?.let { dec ->
                    ...
                    if (canonical != currentPeer) {
                        ...
                        com.bluetalk.android.services.ConversationAliasResolver.unifyChatsIntoPeer(state, canonical, listOf(currentPeer))
                    ...
                    val favoriteRel = try {
                        ...
                        if (noiseKey != null) {
                            com.bluetalk.android.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(noiseKey)
                        } else null
                    ...
                    if (favoriteRel?.isMutual == true) {
                        ...
                        if (noiseHex != currentPeer) {
                            com.bluetalk.android.services.ConversationAliasResolver.unifyChatsIntoPeer(
                                state = state,
            ...
            peers.forEach { pid ->
                try {
                    ...
                    val npub = com.bluetalk.android.favorites.FavoritesPersistenceService.shared.findNostrPubkey(noiseKey)
                    val tempNostrKey: String? = try {
                        if (npub != null) {
                            val (hrp, data) = com.bluetalk.android.nostr.Bech32.decode(npub)
    ...
    private fun unifyChatsIntoPeer(targetPeerID: String, keysToMerge: List<String>) {
        com.bluetalk.android.services.ConversationAliasResolver.unifyChatsIntoPeer(state, targetPeerID, keysToMerge)
    }
     ...
     */
    fun getPeerInfo(peerID: String): com.bluetalk.android.mesh.PeerInfo? {
